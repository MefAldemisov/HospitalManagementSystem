SELECT select2.medical_insurence_number FROM(SELECT select1.medical_insurence_number, select1.`count`, select1.week FROM(SELECT COUNT(medical_insurence_number) as count,medical_insurence_number, date_trunc('week',appointment_timestamp) as week
   ...
   HAVING select1.`count` >= 2) as select2
